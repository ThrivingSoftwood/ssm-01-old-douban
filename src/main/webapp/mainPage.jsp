...
<script>
    ...
    function initHostMovie() {
        ...
        $.ajax({
            type: 'post',
            url: url + "/movie/loadHomePage",
            dataType: 'json',
            data: {},
            success: function (obj) {
                console.log(obj);
                HotNum.append("<span class=\"textcolor_red\">正在热映（" + obj.data.outNow.length + "部）</span>");
                if (obj.data.outNow.length < 8) {
                    ListLength = obj.data.outNow.length;
                } else {
                    ListLength = 8;
                }
                for (var i = 0; i < ListLength; i++) {
                    htmlHot =
                        ...
                        "<a href=\"javascript:void(0)\" target=\"_blank\" data-act=\"playingMovie-click\" data-val=\"" + obj.data.outNow[i].movieId + "\">" +
                        "<div class=\"movie-poster\" style=\"cursor:default;\">" +
                        "<img id=\"moive_picture\" src=\"" + obj.data.outNow[i].moviePicture + "\">" +
                        "<div class=\"movie-overlay movie-overlay-bg\">" +
                        "<div class=\"movie-info\">" +
                        "<div class=\"movie-score\"><i id=\"moive_score\" class=\"integer\">" + obj.data.outNow[i].movieScore + "</i></div>" +
                        "<div class=\"movie-title movie-title-padding\" title=\"" + obj.data.outNow[i].movieCnName + "\">" + obj.data.outNow[i].movieCnName + "</div>" +
                        "</div>" +
                        "</div>" +
                        "</div>" +
                        "</a>" +
                        "<div class=\"movie-detail movie-detail-strong movie-sale\">" +
                        "<a href=\"./buyTickets.jsp?movie_id=" + obj.data.outNow[i].movieId + "\" class=\"active\" target=\"_blank\" data-act=\"salePlayingMovie-click\" data-val=\"{movieid:42964}\">购 票</a>" +
                        "</div>" +
                        "</div>" +
                        "</li>";
                ...
                OnNum.append("<span class=\"textcolor_blue\">即将上映（" + obj.data.upcoming.length + "部）</span>");
                if (obj.data.upcoming.length < 8) {
                    ListLength = obj.data.upcoming.length;
                } else {
                    ListLength = 8;
                }
                for (var i = 0; i < ListLength; i++) {
                    ...
                    htmlOn =
                        "<li>" +
                        "<div class=\"movie-item\">" +
                        "<a href=\"javascript:void(0)\" target=\"_blank\" data-act=\"playingMovie-click\" data-val=\"" + obj.data.upcoming[i].movieId + "\">" +
                        "<div class=\"movie-poster\" style=\"cursor:default;\">" +
                        "<img id=\"moive_picture\" src=\"" + obj.data.upcoming[i].moviePicture + "\">" +
                        "<div class=\"movie-overlay movie-overlay-bg\">" +
                        "<div class=\"movie-info\">" +
                        "<div class=\"movie-score\"><i id=\"moive_score\" class=\"integer\">" + obj.data.upcoming[i].movieScore + "</i></div>" +
                        "<div class=\"movie-title movie-title-padding\" title=\"" + obj.data.upcoming[i].movieCnName + "\">" + obj.data.upcoming[i].movieCnName + "</div>" +
                        "</div>" +
                        "</div>" +
                        "</div>" +
                        "</a>" +
                ...
                for (var i = 0; i < ListLength; i++) {
                    ...
                    notice[i].onclick = function () {
                        layer.alert('‘' + obj.data.upcoming[this.index].movieCnName + '’电影暂未有预告片！', {
                            icon: 0,
                    ...
                    sale[i].onclick = function () {
                        layer.alert('预售‘' + obj.data.upcoming[this.index].movieCnName + '’电影成功！', {
                            icon: 0,
                            offset: clientHeight / 7
                        });
                    }
    ...
    function initBoxOffice(obj) {
        console.log(obj);
        var TempLength;
        if (obj.data.boxOfficeRanking.length > 9) {
            TempLength = 9;
        } else {
            TempLength = obj.data.boxOfficeRanking.length;
        }
        ...
        for (var i = 0; i < TempLength; i++) {
            if (i == 0) {
                BoxOffice.append(
                    "<li class=\"ranking-item ranking-top ranking-index-1\">" +
                    "<a href=\"./movieDetail.jsp?movie_id=" + obj.data.boxOfficeRanking[i].movieId + "\" target=\"_blank\">" +
                    "<div class=\"ranking-top-left\">" +
                    "<i class=\"ranking-top-icon\"></i>" +
                    "<img class=\"ranking-imgs  default-img\" src=\"" + obj.data.boxOfficeRanking[i].moviePicture + "\">" +
                    "</div>" +
                    "<div class=\"ranking-top-right\">" +
                    "<div class=\"ranking-top-right-main\">" +
                    "<span class=\"ranking-top-moive-name\">" + obj.data.boxOfficeRanking[i].movieCnName + "</span>" +
                    "<p class=\"ranking-top-wish\">" +
                    "<span class=\"stonefont\">" + obj.data.boxOfficeRanking[i].movieBoxOffice + "</span>万" +
                    "</p>" +
                    "</div>" +
                    "</div>" +
                    "</a>" +
                ...
                BoxOffice.append(
                    ...
                    "<span class=\"ranking-movie-name\">" + obj.data.boxOfficeRanking[i].movieCnName + "</span>" +
                    "<span class=\"ranking-num-info\">" +
                    "<span class=\"stonefont\">" + obj.data.boxOfficeRanking[i].movieBoxOffice + "</span>万" +
                    "</span>" +
                    "</span>" +
