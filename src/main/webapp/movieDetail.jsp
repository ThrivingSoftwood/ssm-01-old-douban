...
<script>
    ...
    function initBanner() {
        ...
        $.ajax({
            type: 'post',
            url: url + "/movie/loadMovieDetail",
            dataType: 'json',
            data: {
                movieId: movieId
            },
            success: function (obj) {
                StonefontTemp = obj.data.movieDetail.movieBoxOffice;
                StonefontTemp += "万";
                avatar.append("<img class=\"avatar\" src=\"" + obj.data.movieDetail.moviePicture + "\" alt=\"\">");
                movieBriefContainer.append(
                    "<h3 class=\"name\">" + obj.data.movieDetail.movieCnName + "</h3>" +
                    "<div class=\"ename ellipsis\">" + obj.data.movieDetail.movieFgName + "</div>" +
                    "<ul>" +
                    "<li class=\"ellipsis\">" + obj.data.movieDetail.movieType + "</li>" +
                    "<li class=\"ellipsis\">" + obj.data.movieDetail.movieDuration + " / " + obj.data.movieDetail.movieCountry + "</li>" +
                    "<li class=\"ellipsis\">" + obj.data.movieDetail.movieReleaseDate + "</li>" +
                    "<ul>");
                infoNum.append("<span class=\"stonefont\">" + obj.data.movieDetail.movieScore + "</span>");
                scoreNum.append("<span class=\"stonefont\">" + obj.data.movieDetail.movieCommentCount + "</span>人评分");
                stonefontNum.append("<span class=\"stonefont\">" + StonefontTemp + "</span>");
                actionBuyBtn.append("<div class=\"moviedetail\"></div>");
                Introduction.append(
                    "<div class=\"mod-content\">" +
                    "<span class=\"dra\" style=\"font-size:16px;\">" + obj.data.movieDetail.movieDetail + "</span>" +
                    "</div>"
                );
                layui.use('rate', function () {
                    ...
                    rate.render({
                        elem: '#MovieScore'
                        , value: (obj.data.movieDetail.movieScore / 2)
                        , half: true
                        , readonly: true
                    })
                });
    ...
    function intitActor() {
        ...
        $.ajax({
            ...
            success: function (obj) {
                console.log(obj);
                PictureUrl = "static/images/actor/" + obj.data.movieDetail.movieCnName + "/director/" + obj.data.movieDetail.movieDirector + ".jpg";
                // 导演
                celebrityContainer.prepend(
                    ...
                    "</a>" +
                    "<div>" +
                    "<a style=\"text-decoration: none;\" class=\"name\">" + obj.data.movieDetail.movieDirector + "</a>" +
                    "</div>" +
                    "</li>" +
                    "</ul>" +
                    "</div>" +
                ...
                StrActor = obj.data.movieDetail.movieActor.split(',');
                for (var i = 0; i < StrActor.length; i++) {
                    StrRole = StrActor[i].split(':');
                    PictureUrl = "static/images/actor/" + obj.data.movieDetail.movieCnName + "/actor/" + StrRole[0] + ".jpg";
                    // 演员
                    PosName = "&nbsp";
                    if (i == 0) {
                        PosName = "演职人员";
